...
for (u2max in u2maxvec) {
  ...
  DataProb <- rbind(DataProb, data.frame(out1, u = u, j = J1, u2max = u2max, T = max(out1$time)))
}
...
for (u2max in u2maxvec) {
  ...
  DataProb2 <- rbind(DataProb2, data.frame(out1, u = u, j = J1, u2max = u2max, T = max(out1$time)))
}
